...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, async () => {
      ...
      console.log(
        `LinkedIn OAuth: ${
          !!process.env.LINKEDIN_CLIENT_ID ? "Connected" : "Missing"
        }`
      );
      console.log(
        `Google OAuth: ${
          !!process.env.GOOGLE_CLIENT_ID ? "Connected" : "Missing"
        }`
      );
      console.log(
        `Gemini AI: ${!!process.env.GEMINI_API_KEY ? "Connected" : "Missing"}`
      );
      console.log(
        `Telegram Bot: ${
          !!process.env.TELEGRAM_BOT_TOKEN ? "Connected" : "Missing"
        }`
      );
      console.log(
        `Automation: ${
          process.env.ENABLE_AUTOMATION === "true" ? "Enabled" : "Disabled"
        }`
      );
      ...
      try {
        ...
        console.log(
          `Database: PostgreSQL (Users: ${stats.users}, Jobs: ${stats.jobs}, Applications: ${stats.applications})`
        );
      } catch (error) {
