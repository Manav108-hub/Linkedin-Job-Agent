...
export const setupPassportStrategies = () => {
  ...
  // LinkedIn OAuth Strategy - UPDATED SCOPES
  passport.use(
    new LinkedInStrategy(
      {
        ...
        // FIXED: Use OpenID Connect scopes (LinkedIn v2 API)
        scope: ["openid", "profile", "email"], // Changed from deprecated scopes
      },
      (accessToken, refreshToken, profile, done) => {
        try {
          const user = createUserSession(profile, accessToken, "linkedin");
          console.log("LinkedIn user authenticated:", user.name);
